...
import {getAllDocs, getDoc, saveDoc, deleteDoc} from '../firebase'
...
class EditContents extends React.Component {
  constructor(props) {
    super(props)
  ...
  addContent(content, afterContentId) {
    saveDoc(`courses/${this.state.course.id}/contents`, content).then(() => {
      ...
      navigate(`/course/edit-content?courseId=${this.state.course.id}&contentId=${content.id}`)
